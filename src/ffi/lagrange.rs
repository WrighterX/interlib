...
fn core_from_handle(handle: *mut c_void) -> Result<*mut LagrangeCore, &'static str> {
    if handle.is_null() {
        return Err("lagrange handle is null");
    }
    Ok(handle as *mut LagrangeCore)
}

#[unsafe(no_mangle)]
pub extern "C" fn interlib_lagrange_create() -> *mut c_void {
    clear_last_error();
    Box::into_raw(Box::new(LagrangeCore::new())) as *mut c_void
}

#[unsafe(no_mangle)]
pub extern "C" fn interlib_lagrange_destroy(handle: *mut c_void) {
    if handle.is_null() {
        return;
    }
    unsafe {
        drop(Box::from_raw(handle as *mut LagrangeCore));
    }
}

#[unsafe(no_mangle)]
pub extern "C" fn interlib_lagrange_fit(
    handle: *mut c_void,
    x_ptr: *const f64,
    x_len: usize,
    y_ptr: *const f64,
...
#[unsafe(no_mangle)]
pub extern "C" fn interlib_lagrange_update_y(
    handle: *mut c_void,
    y_ptr: *const f64,
    y_len: usize,
) -> i32 {
    ...
    if y_ptr.is_null() {
        return fail("y_ptr must be non-null");
    }
    let y = unsafe { std::slice::from_raw_parts(y_ptr, y_len) };
    if y_len == 0 {
        return fail("y cannot be empty");
    }

    let core = unsafe { &mut *core };
    match core.update_y(y.to_vec()) {
        Ok(()) => success(),
        Err(message) => fail(message),
    }
}
...
#[unsafe(no_mangle)]
pub extern "C" fn interlib_lagrange_eval_many(
    handle: *mut c_void,
    x_ptr: *const f64,
    x_len: usize,
    out_ptr: *mut f64,
...
#[unsafe(no_mangle)]
pub extern "C" fn interlib_lagrange_last_error(buffer: *mut c_char, buffer_len: usize) -> usize {
