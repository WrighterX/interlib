function path = quadraticLibraryPath()
env_path = getenv('INTERLIB_QUADRATIC_LIBRARY');
if ~isempty(env_path)
...
error('interlib:QuadraticLibraryMissing', ...
    'Could not locate the interlib shared library. Set INTERLIB_QUADRATIC_LIBRARY or build the Rust library into target/matlab/debug, target/matlab/release, target/debug, or target/release.');
end
