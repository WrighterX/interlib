...
library_path="${INTERLIB_NEWTON_LIBRARY:-$target_dir/debug/libinterlib.so}"
...
if [[ "$batch_mode" == "1" ]]; then
  ...
  docker run --rm -it \
    -v "$repo_root:/work" \
    -w /work \
    -e INTERLIB_NEWTON_LIBRARY="$library_path" \
    "$image" \
    matlab -licmode onlinelicensing -batch "addpath('matlab'); addpath('matlab/tests'); test_newton"
else
