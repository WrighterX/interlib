...
library_path="${INTERLIB_HERMITE_LIBRARY:-$target_dir/debug/libinterlib.so}"
...
printf 'Using Hermite library: %s\n' "$library_path"
...
if [[ "$batch_mode" == "1" ]]; then
  ...
  docker run --rm -it \
    -v "$repo_root:/work" \
    -w /work \
    -e INTERLIB_HERMITE_LIBRARY="$library_path" \
    "$image" \
    matlab -licmode onlinelicensing -batch "addpath('matlab'); addpath('matlab/tests'); test_hermite"
else
