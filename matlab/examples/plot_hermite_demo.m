function plot_hermite_demo(output_path)
%PLOT_HERMITE_DEMO Plot Hermite interpolation example using values and derivatives.
...
if nargin < 1 || isempty(output_path)
    ...
    output_path = fullfile(output_dir, 'hermite_demo.png');
else
...
fig = figure('Name', 'interlib Hermite MATLAB Demo', 'Color', 'w', 'Visible', 'off');
...
xlabel('x');
ylabel('y');
title('HermiteInterpolator MATLAB Demo (sin(x) with derivatives)');
