function plot_quadratic_demo(output_path)
%PLOT_QUADRATIC_DEMO Plot a simple quadratic interpolation example.
...
if nargin < 1 || isempty(output_path)
    ...
    output_path = fullfile(output_dir, 'quadratic_demo.png');
else
...
fig = figure('Name', 'interlib Quadratic MATLAB Demo', 'Color', 'w', 'Visible', 'off');
...
xlabel('x');
ylabel('y');
title('QuadraticInterpolator MATLAB Demo (sin(x) approx)');
legend({'Interpolated parabolas', 'Data points'}, 'Location', 'best');
hold off;
