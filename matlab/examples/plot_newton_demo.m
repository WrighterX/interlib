function plot_newton_demo(output_path)
%PLOT_NEWTON_DEMO Plot a simple newton interpolation example.
...
if nargin < 1 || isempty(output_path)
    ...
    output_path = fullfile(output_dir, 'newton_demo.png');
else
...
fig = figure('Name', 'interlib Newton MATLAB Demo', 'Color', 'w', 'Visible', 'off');
...
xlabel('x');
ylabel('y');
title('NewtonInterpolator MATLAB Demo (y = x^2)');
legend({'Interpolated polynomial', 'Data points'}, 'Location', 'best');
hold off;
