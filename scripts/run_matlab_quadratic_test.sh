...
library_path="${INTERLIB_QUADRATIC_LIBRARY:-$target_dir/debug/libinterlib.so}"
...
printf 'Using quadratic library: %s\n' "$library_path"
...
if [[ "$batch_mode" == "1" ]]; then
  ...
  docker run --rm -it \
    -v "$repo_root:/work" \
    -w /work \
    -e INTERLIB_QUADRATIC_LIBRARY="$library_path" \
    "$image" \
    matlab -licmode onlinelicensing -batch "addpath('matlab'); addpath('matlab/tests'); test_quadratic"
else
